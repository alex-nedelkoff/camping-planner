...
PARK_RASTERS: dict[str, dict] = {
    "killarney": {
        ...
        # Fallback bounds for the legacy overlay raster.
...
def _raster_info(park: str) -> dict | None:
    """Resolve the raster JPG path + Leaflet bounds for a park."""
...
def get_lakes(park: str):
    """Map overlay layers (lakes, portages, campsites) for a park, keyed by source.

    Public — no auth. The data is already in the public OSM / Jeff's / GPX caches.
    """
